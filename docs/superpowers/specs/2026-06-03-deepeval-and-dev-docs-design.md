...
- Запуск оценок: `pytest -m eval`.
- Живой end-to-end: `pytest -m "eval and live"` (двойной opt-in).
- `tests/evals/conftest.py` делает автоскип, если у активного провайдера нет
  API-ключа — чтобы `pytest -m eval` без ключей аккуратно скипался, а не падал.
...
- LLM недетерминирован: пороги метрик ставим консервативно (например 0.6–0.7),
  тесты помечены как медленные/опциональные.
- Нет ключа → скип, не падение.
...
- что это и зачем opt-in (стоимость, недетерминизм, нужны ключи);
- как запускать: `pytest -m eval`, `pytest -m "eval and live"`, какие env нужны;
...
- `pytest -m eval` при наличии ключа запускает 4 группы метрик и проходит на
  подготовленных голден-данных; без ключа — скипается.
